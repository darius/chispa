...
def main(argv):
    ...
    if argv[1] == 'new':
        ...
        relsearch_ui(argv[2], argv[3:], omit_stopwords)
    else:
...
def relsearch_ui(index_path, query, term_filter, relfind=False):
    terms = set(term_filter(query))
    if not terms:
        sys.stderr.write("The query is empty after stripping stopwords\n")
        sys.exit(1) # XXX was raise, but I don't want a traceback in my tush script
...
def relsearch(index_path, terms):
    for chances in range(4, -1, -1):
        try:
            results = list(ask_index(index_path, 'relevance_search', query=terms))
        except IOError:
            # We're presuming an I/O error is due to a concurrent writer
            # deleting an index file we needed, unless it keeps happening.
            # XXX find a more-selective kind of IOError?
...
def ask_index(index_path, command, **arguments):
    ...
    doc_ids = {path: doc_id for doc_id, (path, _, _) in documents.items()}
    ...
    def doc_length(doc_id):   return documents[doc_id][1]
    def doc_metadata(doc_id): return documents[doc_id][2]
    ...
    def search(query):
        ...
        return map(doc_path,
                   set.intersection(*(set(posting[1] for posting in find(term))
                                      for term in query)))

    def find(term):
        return (posting
                for run in runs
                for posting in find_in_run(run, term))

    def relevance_search(query):
        return ((relevance, doc_path(doc_id))
                for relevance,doc_id in sorted(unordered_search(query),
                                               reverse=True))

    def unordered_search(terms):
        scores = defaultdict(float)
        for term in terms:
            postings = list(find(term))
            scale = log(1 + len(documents) / len(postings)) if postings else 0
            for _, doc_id, count in postings:
                scores[doc_id] += int(count) * scale
        return ((score/doc_length(doc_id), doc_id)
    ...
    def muster_updates(corpus_path):
        ...
        for path in paths:
            ...
            if old_doc_id is None or doc_metadata(old_doc_id) != metadata:
                ...
                documents[doc_id] = (path, None, metadata)
                if debug: print('indexing', basename(path))
                yield path, doc_id

    def tokenize_documents(pairs, term_transform):
        for path, doc_id in pairs:
            term_counter = Counter(term_transform(tokenize_file(path)))
            path, _, metadata = documents[doc_id]
            documents[doc_id] = path, sum(term_counter.values()), metadata
    ...
    if command == 'relevance_search':
        return relevance_search(**arguments)
    elif command == 'update':
        update(**arguments)
    else:
        assert False
